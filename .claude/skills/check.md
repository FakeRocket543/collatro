...
根據原文和 Step 1 的實體，列出所有可驗證的事實主張。每則主張標出：
- 原文段落（引用）
- 涉及的實體（人/機構/地/數字/時間）
...
- 字體大小規範：
  - 橫式：主張 text-xl、比對內容 text-base、來源 text-sm
  - 直式：主張 text-2xl、比對內容 text-lg、來源 text-base
  - KG tags：text-sm（橫式）/ text-base（直式）
- 頂部：Collatro logo + 「事實比對卡」
- 原始主張區塊（深色卡片）
- KG tags（Wikidata 結構化資料做成彩色 pill badges：出生/死亡/職業/國籍等）
- 判定結果（大字，紅/綠/黃）
- 比對差異（NER/數字/時間 分類標籤 + 主張 vs 證據）
